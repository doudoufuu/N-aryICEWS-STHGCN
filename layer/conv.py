# [MODIFIED] Restored HypergraphTransformer from original STHGCN implementation.
import math
import torch
...
class HypergraphTransformer(MessagePassing):
    r"""Hypergraph Conv containing relation transform、edge fusion(including time fusion)、self attention and gated residual connection(or skip connection).
    ...
    # updated signature to avoid TorchGeometric template conflicts.
    def forward(  # [MODIFIED]
        self,
        x: Union[Tensor, OptPairTensor],
        edge_index: Adj,
        edge_time_embed: Optional[Tensor],
        edge_dist_embed: Optional[Tensor],
        edge_type_embed: Optional[Tensor],
        edge_attr_embed: Optional[Tensor],
    ):
        ...
        if isinstance(edge_index, SparseTensor):
            out = self.propagate(
                ...
                edge_attr_embed=self._ensure_tensor(edge_attr_embed, edge_index.nnz()),
                edge_time_embed=self._ensure_tensor(edge_time_embed, edge_index.nnz()),
                edge_dist_embed=self._ensure_tensor(edge_dist_embed, edge_index.nnz()),
            ...
            out = self.propagate(
                ...
                edge_attr_embed=self._ensure_tensor(edge_attr_embed, edge_index.size(1)),
                edge_time_embed=self._ensure_tensor(edge_time_embed, edge_index.size(1)),
                edge_dist_embed=self._ensure_tensor(edge_dist_embed, edge_index.size(1)),
                edge_type_embed=edge_type_embed,
                have_query_feature=self.have_query_feature,
                size=None
            )
        ...
        if self.residual_fusion_mode == 'concat':
            ...
            if out.size(0) != x[1].size(0):
                if out.size(0) < x[1].size(0):
                    pad = out.new_zeros(x[1].size(0) - out.size(0), out.size(1))
    ...
    #ensure optional encodings default safely.
    def message(  # [MODIFIED]
        self,
        x: OptPairTensor,
        edge_attr_embed: Optional[Tensor],
        edge_time_embed: Optional[Tensor],
        ...
        edge_time_embed = self._default_time(edge_time_embed, x_j)   
        edge_dist_embed = self._default_dist(edge_dist_embed, x_j)   
        edge_attr_embed = self._default_attr(edge_attr_embed, x_j)   
        ...
        if self.trans_flag:
            ...
            x_j_raw = x_j
        if edge_type_embed is not None and edge_type_embed.numel() > 0:  # [MODIFIED]
        ...
        if edge_attr_embed is not None:  # [MODIFIED]
    ...
    @staticmethod   
    def _ensure_tensor(src: Optional[Tensor], edge_count: int) -> Optional[Tensor]:
        if src is None:
            return None
        if src.size(0) == edge_count:
            return src
        return src[:edge_count]

    def _default_time(self, embed: Optional[Tensor], x_j: Tensor) -> Tensor:   
        if embed is not None:
            return embed
        if self.time_fusion_mode is None or self.time_embed_dim == 0:
            return x_j.new_zeros(x_j.size(0), 0)
        return x_j.new_zeros(x_j.size(0), self.time_embed_dim)

    def _default_dist(self, embed: Optional[Tensor], x_j: Tensor) -> Tensor:   
        if embed is not None:
            return embed
        if self.time_fusion_mode is None or self.dist_embed_dim == 0:
            return x_j.new_zeros(x_j.size(0), 0)
        return x_j.new_zeros(x_j.size(0), self.dist_embed_dim)

    def _default_attr(self, embed: Optional[Tensor], x_j: Tensor) -> Optional[Tensor]:   
        if embed is not None:
            return embed
        if self.edge_fusion_mode == 'concat':
            if self.edge_dim is None or self.edge_dim == 0:
                return x_j.new_zeros(x_j.size(0), 0)
            return x_j.new_zeros(x_j.size(0), self.edge_dim)
        if self.edge_fusion_mode == 'add':
            return x_j.new_zeros_like(x_j)
        return None
