...
def _split_with_coverage(df: pd.DataFrame, cfg: Cfg) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    return df, report
def create_event_chains(df: pd.DataFrame, *, window_days: int = 7, seed: int = 42) -> pd.Series:
    """
    按 Source_name + 7 天时间窗口组织事件链。
    """
    print("[Event Chains] 基于 Source_name 和时间窗口创建事件链...")
    if "Source_name_encoded" not in df.columns:
        raise ValueError("缺少 Source_name_encoded 列，无法构建事件链。")

    df_sorted = df.sort_values("UTCTimeOffsetEpoch")
    time_window = window_days * 24 * 60 * 60
    print(f"[Event Chains] 时间窗口: {window_days} 天 ({time_window} 秒)")

    sources = sorted([s for s in df_sorted["Source_name_encoded"].dropna().unique().tolist()])
    print(f"[Event Chains] Source 数量: {len(sources)}")

    event_chains: Dict[int, str] = {}
    for src_idx, source in enumerate(sources, start=1):
        source_events = df_sorted[df_sorted["Source_name_encoded"] == source]
        if source_events.empty:
            continue
        source_events = source_events.sort_values("UTCTimeOffsetEpoch")
        current_window_start = int(source_events.iloc[0]["UTCTimeOffsetEpoch"])
        current_chain: List[int] = []
        chain_id_counter = 0

        for idx, row in source_events.iterrows():
            event_time = int(row["UTCTimeOffsetEpoch"])
            if event_time - current_window_start <= time_window:
                current_chain.append(idx)
            else:
                if current_chain:
                    chain_id = f"{int(source)}_{chain_id_counter}"
                    for event_idx in current_chain:
                        event_chains[event_idx] = chain_id
                    chain_id_counter += 1
        ...
        if src_idx % 100 == 0:
            print(f"[Event Chains] 已处理 Source {src_idx}/{len(sources)}")
...
def filter_symmetric_events(events_df: pd.DataFrame, target_country: int) -> pd.DataFrame:
    ...
    for event_key, group in symmetric_groups:
        ...
        if len(group) == 1:
            # 单个事件，直接保留
            event = group.iloc[0]
            if target_country is None or event['Source_Country_encoded'] == target_country:
                filtered_events.append(event)
        else:
            # 多个事件，检查是否存在主体互换且Event_type对称的对称事件
            used = set()
            ...
            for i, row_i in enumerate(group_list):
                ...
                for j, row_j in enumerate(group_list):
                    if i == j or j in used:
                        continue
                    # 检查主体互换
                    if (row_i['Source_name_encoded'] == row_j['Target_name_encoded'] and
                        row_i['Target_name_encoded'] == row_j['Source_name_encoded']):
                        # 检查Event_type对称
                        et_i = row_i['Event_type']
                        et_j = row_j['Event_type']
                        if symmetric_type_map.get(et_i, None) == et_j:
                            # 检查时间、地点、Intensity相同
                            loc_i = row_i.get('Location_encoded', 'unknown')
                            loc_j = row_j.get('Location_encoded', 'unknown')
                            if (row_i['UTC_time'] == row_j['UTC_time'] and
                                loc_i == loc_j and
                                row_i['Intensity'] == row_j['Intensity']):
                                # 是对称事件，只保留以目标国家为源且Event_type较小的那条
                                symmetric_count += 1
                                if target_country is None:
                                    # 全局过滤，保留Event_type较小的
                                    if et_i <= et_j:
                                        filtered_events.append(row_i)
                                    else:
                                        filtered_events.append(row_j)
                                else:
                                    # 指定国家过滤
                                    if row_i['Source_Country_encoded'] == target_country and et_i <= et_j:
                                        filtered_events.append(row_i)
                                    elif row_j['Source_Country_encoded'] == target_country and et_j < et_i:
                                        filtered_events.append(row_j)
                                    else:
                                        # 都不是以目标国家为源，保留Event_type较小的
                                        if et_i <= et_j:
                                            filtered_events.append(row_i)
                                        else:
                                            filtered_events.append(row_j)
                ...
                if not found_symmetric and i not in used:
                    # 没有找到对称事件，只做去重，保留以目标国家为源的
                    if target_country is None:
                        # 全局过滤，保留第一个
                        filtered_events.append(row_i)
                    elif row_i['Source_Country_encoded'] == target_country:
                        filtered_events.append(row_i)
                    elif row_i['Target_Country_encoded'] == target_country:
                        filtered_events.append(row_i)
                    else:
                        filtered_events.append(row_i)
                    used.add(i)
...
def create_event_key(row):
    ...
    # 创建对称事件的特征键
    # 使用较小的实体ID在前，确保对称事件有相同的键
    if source < target:
        entity_pair = f"{source}_{target}"
    else:
        entity_pair = f"{target}_{source}"
...
def preprocess_csv_events(data_path: bytes, preprocessed_path: bytes, cfg: Cfg) -> pd.DataFrame:
    # 检查预处理目录是否存在
    # keep passed-in preprocessed_path
    ...
    if not progress['raw_loaded']:
        ...
        print("[CSV Events] 跳过CSV文件读取（已完成）")
        # 需要从某个中间文件加载df，这里假设保存了中间结果
        # df = pd.read_pickle(osp.join(preprocessed_path, 'temp_df.pkl'))
    ...
    if not progress['time_converted']:
        ...
        with open(progress_file, 'wb') as f:
            pickle.dump(progress, f)
        df.to_pickle(osp.join(preprocessed_path, 'temp_df.pkl'))  # 保存中间结果
    else:
        print("[CSV Events] 跳过时间转换（已完成）")
    ...
    if not progress['filtered']:
        ...
        with open(progress_file, 'wb') as f:
            pickle.dump(progress, f)
        df.to_pickle(osp.join(preprocessed_path, 'temp_df.pkl'))
    else:
        print("[CSV Events] 跳过对称事件过滤（已完成）")
    ...
    if not progress['event_chains_created']:
        ...
        seed_val = int(getattr(cfg.run_args, "seed", 42) or 42)
        df['EventChain_id'] = create_event_chains(df, seed=seed_val)
        ...
        print("[CSV Events] 跳过事件链创建（已完成）")

    progress['check_ins_calculated'] = False


        # 6.1 筛选小规模数据集（保证事件链不拆分）
    if not progress.get('small_dataset_selected'):
        ...
        # 按时间或ID排序，优先保留前面的链
        chain_sizes = chain_sizes.sort_values(by='EventChain_id')  # 或者按时间字段
        ...
        print("[CSV Events] 跳过小规模数据集筛选（已完成）")
    ...
    if not progress['check_ins_calculated']:
        ...
        print(f"[CSV Events] check_ins_id唯一值数量: {df['check_ins_id'].nunique()},df['check_ins_id'].max(): {df['check_ins_id'].max()}")
        progress['check_ins_calculated'] = True
        with open(progress_file, 'wb') as f:
            pickle.dump(progress, f)
        df.to_pickle(osp.join(preprocessed_path, 'temp_df.pkl'))
    else:
        print("[CSV Events] 跳过check_ins_id计算（已完成）")
    ...
    # else:
    #     print("[CSV Events] 跳过字段编码（已完成）")
    #     with open(osp.join(preprocessed_path, 'label_encoding.pkl'), 'rb') as f:
    ...
    if not progress['encoded']:
        ...
        for i, field in enumerate(fields_to_encode):
            ...
            # 如果是整数型字段，直接计算 padding
            if np.issubdtype(df[field].dtype, np.integer):
                # 如果是整数型字段，也统一用类别数来计算 padding
                padding = len(df[field].unique())
                encoders[field] = (None, padding)
                print(f"  已为数值型字段设置padding: {padding} (耗时: {time.time()-start_time:.2f}s)")
                ...
                if field == 'ID':
                    ...
                    # 1. 全量收集唯一值（train/val/test 都包括）
                    all_ids = df[field].astype(str).unique()
                    le.fit(all_ids)  # 这样保证不会有 unseen label
                    ...
                    # 普通字段只用训练集 fit
                    le.fit(df[df['SplitTag'] == 'train'][field].astype(str))
                    # transform 时要兼容 unseen label
        ...
        # 保证 EventChain_id 连续且从 0 开始（避免后续构图断号）
        if 'EventChain_id' in df.columns:
            unique_chain_ids = sorted(pd.to_numeric(df['EventChain_id'], errors='coerce').dropna().astype(int).unique().tolist())
            mapping = {old: new for new, old in enumerate(unique_chain_ids)}
            df['EventChain_id'] = df['EventChain_id'].map(mapping).astype(np.int64)
            # 覆盖编码器项，使用连续的计数作为 padding
            encoders['EventChain_id'] = (None, len(unique_chain_ids))
            print(f"[CSV Events] EventChain_id 映射为连续ID: 0~{len(unique_chain_ids)-1}，原最大: {unique_chain_ids[-1] if unique_chain_ids else 'NA'}")
    ...
    # 最终清理
    if osp.exists(osp.join(preprocessed_path, 'temp_df.pkl')):
        os.remove(osp.join(preprocessed_path, 'temp_df.pkl'))
...
def preprocess(cfg: Cfg):
    ...
    # 使用参数构造预处理数据路径
    preprocessed_path = osp.join(data_path, f'preprocessed_5')
